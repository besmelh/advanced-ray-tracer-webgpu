...
// Trace ray and return the resulting contribution of this ray
// this function implements recursive reflection rays using iteration 
fn get_pixel_color_recur(ray: Ray) -> vec3<f32> {
  // Sample the environment map regardless of whether the ray hits an object.
  let background_texture = sample_cubemap(ray.dir);

  // keep track of how many recursive reflections we find
  var reflection_exists = 0;
  ...
  // for (var i: i32 = 0; )
  // refl_rays 
  // we already have an initialized array to save the values of the refelction rays -> refl_rays
  // note: may not be needed
  for (var i: i32 = 0; i < max_refl_rays; i++){
    refl_rays[i] = ray;
  }
  ...
  else
  {
    ...
    var cur_rec = rec;
    var cur_reflection_ray = reflection_ray;

    // check recursive reflections
    for (var i: i32 = 0; i < max_refl_rays; i++){
      // if the material we hit is reflective
      if (cur_rec.hit_material.reflectivity > 0){
        reflection_exists += 1;
        refl_rays[i] = compute_glossy_reflection_ray(ray, rec);
      } else {
        break;
      }
    }
  ...
  // an array to store the reflection_textures
  var reflection_textures: array<vec3<f32>, 5>;
 for (var i: i32 = 0; i < max_refl_rays; i++){
  reflection_textures[i] = sample_cubemap(refl_rays[0].dir);
 }
  // if the surface we found is reflective, add the reflection texture
  if (reflection_exists){
    var ref_rec = trace_ray(reflection_ray);
...
var<private> max_refl_rays: i32 = 5;
var<private> refl_rays: array<Ray, 5>;
