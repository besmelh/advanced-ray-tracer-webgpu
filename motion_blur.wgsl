...
// global variable
  ...
  var num_samples= i32(16);
  var pixel_color = vec3<f32>(0.0, 0.0, 0.0);

  world_spheres[0].start_center = vec3<f32>(-0.125, 0.30, 0.35);//world_spheres[0].center - vec3<f32>(0.2,0.0,0.0);
  world_spheres[0].end_center = vec3<f32>(-0.125, 0.1, 0.35);//world_spheres[0].center + vec3<f32>(0.2,0.0,0.0);
   
  for (var s: i32 = 0; s < (num_samples); s=s+1) {
    let u_offset = (f32(s) + rnd()) / f32(num_samples);
    let v_offset = (f32(s) + rnd()) / f32(num_samples);

    let ui = _left + (_right - _left) * ((pixel_position.x + u_offset) / (image_resolution.x));
    let vi = _bottom + (_top - _bottom) * ((pixel_position.y + v_offset) / (image_resolution.y));
 
    let time_sample = 0.5 * f32(s) / f32(num_samples);
    //world_spheres[0].start_center = vec3<f32>(-0.150, 0.25, 0.35);//world_spheres[0].center - vec3<f32>(0.2,0.0,0.0);
    //world_spheres[0].end_center = vec3<f32>(-0.100, 0.25, 0.35);//world_spheres[0].center + vec3<f32>(0.2,0.0,0.0);
    world_spheres[0].center = world_spheres[0].start_center * (1.0 - time_sample) + world_spheres[0].end_center * time_sample;

    var ray: Ray = get_ray(camera, ui, vi,time_sample);
...
fn setup_light()
{
  light.position = vec3<f32>(3.0, 3.0, 1.0); //use it as corner
  light.e1 = vec3<f32>(1.0,0.0,0.0);
  light.e2 = vec3<f32>(0.0,1.0,0.0);
...
fn get_ray(camera:Camera,ui:f32,vj:f32, time:f32)->Ray{
  ...
  //ray.orig = ray.orig + camera.origin;//world_spheres[0].start_center * (1.0 - time) + world_spheres[0].end_center * time;
...
fn compute_shading(ray: Ray, rec:HitRecord)-> vec3<f32>{
    ...
    let rl1 = rnd();
    let rl2 = rnd();
    let rndLight = light.position + rl1*light.e1 + rl2*light.e2;
    let shadowDir = normalize(rndLight - rec.p);
...
fn trace_ray(ray: Ray) -> HitRecord{
  ...
  for (var i: i32 = 0; i < world_spheres_count; i++) {
    //world_spheres[i].center = world_spheres[i].start_center * (1.0 - Time) + world_spheres[i].end_center * Time;
